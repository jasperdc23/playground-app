    ...
    <div className="space-y-4">
      {sections.map((section) => (
        <div key={section.label}>
          <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-widest px-3 mb-1">{section.label}</p>
          <div className="space-y-0.5">
            {section.links.map((link) => {
              const active = pathname === link.href;
              const accentColor = (link as any).color ?? "#818cf8";
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  style={
                    active
                      ? { background: `${accentColor}15`, border: `1px solid ${accentColor}35`, color: "#ffffff" }
                      : { border: "1px solid transparent", color: "#6b7280" }
                  }
                  className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 hover:bg-white/[0.04] hover:text-white"
                >
                  <span style={{ color: active ? accentColor : "inherit" }}>{link.icon}</span>
                  {link.label}
                  {active && <span className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: accentColor }} />}
                </Link>
              );
            })}
          </div>
