...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    { label: "Auth Provider",  value: provider === "email" ? "Email" : provider.charAt(0).toUpperCase() + provider.slice(1), sub: "Signed in method", color: "text-indigo-400" },
    { label: "Member Since",   value: new Date(user?.createdAt ?? Date.now()).getFullYear().toString(), sub: "Account created", color: "text-violet-400" },
  ];
    ...
    <div className="max-w-4xl space-y-8">
      <div className="flex items-center gap-5">
        {image ? (
          ...
          <img src={image} alt="avatar" className="w-16 h-16 rounded-2xl ring-2 ring-indigo-500/40" />
        ) : (
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Good day, {name.split(" ")[0]} 👋</h1>
          <p className="text-gray-400 mt-0.5">{email}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((s, i) => (
          <div key={s.label} className={`glass rounded-2xl p-5 space-y-1 animate-fade-in animate-delay-${(i + 1) * 100}`}>
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { icon: "⚙️", label: "Settings",  desc: "Update your profile and preferences", href: "/dashboard/settings" },
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-3">
          {[
            ...
            { icon: "✅", text: "Account verified",             time: "Today"    },
